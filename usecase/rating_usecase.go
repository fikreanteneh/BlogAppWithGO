...
// GetRatingsByUserID implements usecase.RatingUseCase.
func (r *RatingUseCase) GetRatingsByUserID(currUser *model.AuthenticatedUser, dto any, param *model.IdParam) (*[]*model.RatingInfo, string, error) {
	rates, err := r.ratingRepository.GetRatingByUserID(r.context, param.ID)
	if err != nil {
		return nil,"Ratings Fetching Failed", err
	}

	var blogRatings []*model.RatingInfo
	for _, rate := range *rates {
		blog, err := r.blogRepository.GetByID(r.context, rate.BlogID)
		if err != nil {
			return nil, "Blog Not Found", err
		}
		blogRatings = append(blogRatings, &model.RatingInfo{
			BlogID: blog.BlogID,
			Title:  blog.Title,
	...
	return &blogRatings, "Ratings Fetched Successfully", nil
